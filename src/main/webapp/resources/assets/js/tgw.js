...
function buyTokenConfirmation() {
	...
	if (ether != undefined && ether != '') {
		if(confirm ("Are you sure you want to buy FYN for " + ether + " Ether?")) {
			$('#buyTokenForm').submit();
		}
	} else {
		alert ("Please enter the ether amount to buy FYN!");
	}
}
...
function sendTokenConfirmation() {
	...
	if (toaddress != undefined && toaddress != '' && tokenamount != undefined && tokenamount != '') {
		if (tokenamount % 1 != 0) {
			alert ("Please enter a valid FYN amount!");
		} else {
			if(confirm ("Are you sure you want to send " + tokenamount + " FYN to " + toaddress + "?")) {
				$('#sendTokenForm').submit();
			}
		}
		
	} else {
		alert ("Please fill the recipient address and FYN amount!");
	}
}